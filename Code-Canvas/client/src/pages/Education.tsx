...
import { Loader2, Code, LayoutGrid, GraduationCap, Calendar } from "lucide-react";
...
export default function Education() {
  ...
  const codeString = JSON.stringify(formattedEducation, null, 2);
    ...
    <div className="h-full flex flex-col">
      ...
      >
        <div className="flex items-center justify-between mb-8 px-2">
          <h2 className="text-2xl font-bold text-[#61afef] flex items-center gap-3">
            ...
            >
              <div className="space-y-6 p-2 max-w-4xl">
                {education?.map((edu, index) => (
                  ...
                  >
                    <div className="absolute left-0 top-2 w-6 h-6 rounded-full bg-[#61afef]/20 flex items-center justify-center">
                      <div className="w-2.5 h-2.5 rounded-full bg-[#61afef]" />
                    </div>
                    
                    <div className="bg-[#282c34] rounded-xl p-6 border border-[#3e4451] hover:border-[#61afef] transition-all group">
                      <div className="flex items-center gap-2 mb-3">
                        <span className="flex items-center gap-1 text-[#56b6c2] text-xs">
                          <Calendar size={12} />
                          {edu.year}
                        </span>
                      </div>
                      
                      <h3 className="text-xl font-semibold text-[#d4d4d4] mb-2 group-hover:text-[#61afef] transition-colors">
                        {edu.institution}
                      </h3>
                      
                      <p className="text-[#e5c07b] text-sm font-medium">
                        {edu.degree}
                      </p>
                    </div>
                  </motion.div>
