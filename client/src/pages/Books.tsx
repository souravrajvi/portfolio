              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-2">
                {books?.map((item, index) => (
                  <motion.div
                    ...
                    className="bg-[#282c34] rounded-lg overflow-hidden border border-[#3e4451] hover:border-[#98c379] transition-all group hover:shadow-2xl hover:-translate-y-1"
                  >
                    <div className="h-56 bg-gradient-to-br from-[#98c379]/20 to-[#d19a66]/20 flex items-center justify-center relative overflow-hidden shadow-inner">
                      {item.imageUrl ? (
                        <>
                          <img 
                            src={item.imageUrl} 
                          ...
                          />
                          <div className="absolute inset-0 bg-gradient-to-t from-[#282c34]/40 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                        </>
                      ) : (
                        <Book size={48} className="text-[#98c379]/40" />
                      )}
                      {item.genre && (
                        <span 
                          className="absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-semibold backdrop-blur-sm"
                          style={{ 
                            backgroundColor: `${genreColors[item.genre] || '#98c379'}30`,
                            color: genreColors[item.genre] || '#98c379',
                            border: `1px solid ${genreColors[item.genre] || '#98c379'}60`
                          }}
                        >
                          {item.genre}
                        </span>
                    ...
                    <div className="p-5 bg-gradient-to-b from-[#282c34] to-[#21252b]">
                      <h3 className="text-[#d4d4d4] font-bold text-base mb-1 group-hover:text-[#98c379] transition-colors line-clamp-2">
                        {item.title}
                      </h3>
